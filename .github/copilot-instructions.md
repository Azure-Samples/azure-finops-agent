...
│   ├── KnowledgeTools.cs   # QueryKnowledge — read-only list/search/get over the user's persistent org knowledge (used in index-mode injection for large knowledge bases)
...
Lets a user save persistent context about their environment once (subscription→app mappings, cost-center owners, SLA/RTO/RPO targets, tagging conventions, fiscal calendar, custom analysis instructions); the agent applies it automatically across every conversation. **Entra-only** (anonymous sessions excluded). See `docs/knowledge-base.md` (technical) and `docs/knowledge-authoring-guide.md` (user-facing).

- **`KnowledgeStore`** (`src/Dashboard/Services/KnowledgeStore.cs`) — per-user file store at `$COPILOT_HOME/knowledge/{userId}/knowledge.json` (durable `/home` mount, encrypted at rest, atomic temp+move writes, per-user write lock). `KnowledgeArticle` { Id (8-char hex `^[a-f0-9]{8}$`), Title, Category, Content, UserId, timestamps, Active }. Limits: 20 articles/user, 10K chars/article, 50K total, 120-char title. Categories: `subscriptions, cost_centers, instructions, architecture, sla, custom`. `BuildContextBlock(userId, sessionId)` builds the prompt-injection block.
- **`KnowledgeEndpoints`** (`src/Dashboard/Endpoints/KnowledgeEndpoints.cs`) — `GET/POST /api/knowledge`, `GET/PUT/DELETE /api/knowledge/{id}`, `POST /api/knowledge/import` (text files: csv/tsv/txt/json/md/log, truncated to limit). Entra-gated: `userId` derived server-side from session, never from request; anonymous → HTTP 401; validation errors → HTTP 400. Registered via `app.MapKnowledgeEndpoints()`.
- **Prompt injection** (`ChatEndpoints.cs`) — knowledge block is composed after `activeSessionId` is known and prepended in order CONTEXT → KNOWLEDGE → UPLOADED FILES → user prompt.
- **`KnowledgeTools`** (`QueryKnowledge`) — read-only list/search/get, registered per-user in `CopilotSessionFactory.GetOrCreateUserTools` via `new KnowledgeTools(uid)`.
- **Token-cost model (two tiers)**: Tier 1 — inject once per session, re-inject only on content-hash change or every 10 turns, zero I/O when the user has no articles. Tier 2 — above `FullInjectionCharBudget` (4,000 chars active content), inject a compact `[ORGANIZATIONAL KNOWLEDGE INDEX]` instead of full text and let the model lazy-pull via `QueryKnowledge`. The `SystemPrompt` in `CopilotSessionFactory.cs` has an `## Organizational Knowledge` section instructing the model to treat the block as ground truth (never overriding the security model) and to call `QueryKnowledge` when only the index is present.
- **Retention**: knowledge persists for returning Entra users — **not** auto-purged. Logout ≠ account deletion (knowledge survives logout). Users delete their own articles; `KnowledgeStore.DeleteAllForUser` exists for a future account-deletion flow.
- **Frontend**: `ChatView.vue` has a collapsible "Knowledge Base" sidebar section (shown when `azureConnected`), an editor modal (title/category/content with char count), on/off toggle, delete, and text-file import. Loaded on connect, cleared on disconnect/revoke/logout.
