...
- **AI**: GitHub Copilot SDK (`GitHub.Copilot.SDK` 1.0.6; the bundled CLI runtime is pinned to `@github/copilot` **1.0.68** in `Dashboard.csproj` via `<CopilotCliVersion>` because the SDK's default `1.0.69` is unpublished on npm — left unpinned the build-time runtime download 404s locally **and** in ACR) with BYOK (Bring Your Own Key) using Azure OpenAI via Entra ID bearer tokens. Sessions managed via `CopilotClient` / `CopilotSession`. Default model `gpt-5.6-sol` (GlobalStandard, **Priority processing** service tier). Reasoning effort is configurable via `AzureOpenAI:ReasoningEffort` (default `medium` — best time-to-first-token/quality balance for GPT-5.6; `high` and `xhigh` are opt-in, and `xhigh` was measured at 8+ min per LLM round-trip in production). **Production runs `medium`** — it was on `high` until 2026-08-31, which measured 34.8s to first token on a VM price comparison vs 22.2s at `medium` (the tool itself returned at 5.8s, so ~29s was post-tool reasoning). **Latency rule of thumb, measured against this deployment: a large system prompt costs almost nothing (0.56s TTFT with 5KB of instructions vs 0.71s bare), but every extra LLM round-trip costs seconds.** Optimise by removing round-trips, not by trimming prompts. Trivial turns (greetings/small talk, see `IsTrivialPrompt`) get `TrivialTurnDirective` prepended so they answer in ONE round-trip with no tools and no tables — `hello` went from 6.25s to 1.48s on prod. The Copilot CLI's `task` sub-agent tool is excluded from sessions (`ExcludedTools`) — one `task` call looped for 8 minutes in production. The Copilot CLI provides built-in tools (file operations, bash, grep, glob, web fetch, memory) — custom tools handle Azure-specific APIs. Multi-session per user: each user can keep many conversations, listed in the right sidebar; the SDK auto-disconnects idle sessions after 30 min (`SessionIdleTimeoutSeconds = 1800`) while preserving on-disk state for resume.
...
- **One session setup at a time per user**: `CopilotSessionFactory` holds a `SemaphoreSlim` gate per user id (`_userSessionGates`) around `GetCurrentOrCreateAsync` / `GetOrResumeAsync`; the inner `...CoreAsync` variants assume the gate is already held so it is never re-entered. Without it, `/api/chat/warmup` and the user's first prompt race, both miss `CurrentSessionId`, both create a session, and the loser's resume throws `Session '...' is already tracked by this client` — which produced THREE sessions for a single "hi". The gate is also what makes warmup useful (its session is now the one the prompt reuses), cutting cold first token from ~8.0s to ~4.7s.
- **`DefaultAzureCredential` excludes ManagedIdentity when not running in Azure** (`IDENTITY_ENDPOINT` / `MSI_ENDPOINT` / `WEBSITE_INSTANCE_ID`). Azure.Identity surfaces MSAL's `managed_identity_all_sources_unavailable` as a FATAL `AuthenticationFailedException`, so leaving it in the chain makes every local run fail at ManagedIdentity instead of falling through to the Azure CLI credential.
