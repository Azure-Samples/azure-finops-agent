...
- Evaluation repair handoffs (`repair-plan.json`) bind failed cases to the candidate revision and suite. `EVAL_ONLY_IDS` is local diagnosis only and can never pass the gate, even with 100 passing cases. Require a reviewed fix and full new-candidate run; never retry unchanged failures until green. Classification applies locally too, and raw/partial runner results stay outside published artifact directories.
