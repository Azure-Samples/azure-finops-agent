...
- **AI**: GitHub Copilot SDK (`GitHub.Copilot.SDK` 1.0.0-beta.3) with BYOK (Bring Your Own Key) using Azure OpenAI via Entra ID bearer tokens. Sessions are **stateless server-side** — each user turn creates a fresh `CopilotSession` via `CopilotSessionFactory.CreateOneShotAsync`, runs one prompt with the browser-provided history replayed as a composed prompt, then disposes. No server-side conversation persistence. Reasoning effort set to `xhigh`. The Copilot CLI provides built-in tools (file operations, bash, grep, glob, web fetch, memory) — custom tools handle Azure-specific APIs. Multi-conversation: each user can keep many conversations in browser IndexedDB and switch between them; the sidebar lists conversations stored on the current device only.
...
- **Chat persistence**: Conversation history lives **only in the user's browser** (IndexedDB, via `idb`). The server has no DB, no file store, no chat session state. See `src/Dashboard/frontend/src/lib/sessionStore.js` for the schema. Switching browser/device or clearing site data removes history.
...
- **Stateless chat backend.** GitHub Copilot SDK sessions are one-shot: `CopilotSessionFactory.CreateOneShotAsync(userId)` creates a session per turn, the caller streams the response, then disposes. Conversation history lives only in the browser (IndexedDB) and is replayed on each turn via the `history` field of the `/api/chat` POST body. The server stores nothing about past turns — no `LiveSessions`, no `SessionTitles`, no `/home/copilot` mount, no janitor TTL sweep.
...
- BYOK authentication: `DefaultAzureCredential` generates bearer tokens for `https://cognitiveservices.azure.com/.default`, passed to the Copilot SDK via `ProviderConfig.BearerToken`. Tokens are cached (~1h) and refreshed on next session creation if near expiry — there is no need to recycle long-lived sessions because no session lives longer than one turn.
- The backend streams responses via **Server-Sent Events (SSE)** using session event subscriptions (`session.On()`) — deltas, tool starts/completions, chart events, errors. Title generation (one tiny completion after the first turn) is emitted as a `session_title` SSE event for the browser to persist.
- The Vue frontend consumes SSE and renders streaming text (character-by-character animation), tool call status in the sidebar, intent text next to the AI avatar, ECharts visualizations inline, and a vertical-split right sidebar (Agent on top, browser-local Conversations on bottom).
- Data should be retrieved from APIs at runtime — the agent stores dynamic FAQ entries to disk for SEO but no user data.
- **Chat history endpoints**: there are none. The browser owns it via `frontend/src/lib/sessionStore.js` (sessions store + messages store, keyed by browser-generated UUID).
