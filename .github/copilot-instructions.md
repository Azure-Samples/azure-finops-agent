...
- Generated `az graph query` commands use `--graph-query`/`-q` for KQL and `--query` only for JMESPath output selection. Failed queries and invalid/missing counts remain explicit errors, not zero results. Artifact packaging never validates or executes the script. `FindIdleResources` returns a host-built read-only `revalidationScript` from its exact scan filters; reuse it rather than letting the model author revalidation KQL.
...
- Graph query options are endpoint-specific: `subscribedSkus` accepts only `$select`, and its response carries a host-computed `_licenseSummary` whose seat totals answers copy rather than re-add; Copilot usage functions do not accept `$filter`, `$top` or `$select`. Prefer current `/v1.0/copilot/reports/` routes and preserve CSV versus beta JSON, report version/period, source refresh date and licensed-user coverage.
...
- Advisor recommendation/savings-by-impact questions use `GetAdvisorCostRecommendations` once with the full subscription scope: host-grouped impact levels (including empty ones), merged term/lookback alternatives, host-built headline/table and no combined savings total. It and Crawl savings evidence render each opportunity's Advisor lastUpdated source date in the host table; retrieval time never substitutes for it.
- Resource counts by type use `GetResourceInventory` once: one host-built Resource Graph aggregate with ranked types, shares, total and a host-built headline/table, so the model neither authors that KQL nor post-aggregates it with `QueryToolResult`.
