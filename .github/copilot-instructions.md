...
- **GraphQueryTools** (`QueryGraph`) is **read-only — GET only** (write methods are rejected at the code level in addition to the read-only Graph scopes). calls Microsoft Graph API using `TokenContext.GraphToken`. Used for license inventory, M365 usage reports (Exchange, Teams, OneDrive, SharePoint), M365 Copilot seat usage, M365 app-level usage, Intune device management, directory objects, org structure for FinOps chargebacks.
