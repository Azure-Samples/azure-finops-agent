...
- Empty `QueryToolResult` select/groupBy objects mean omitted projection/grouping. Ungrouped aggregates remain available in `totals` even with `limit=0`; grouped amounts, particularly different currencies, never become an unrequested grand total. A query array (up to 8) answers several views of one handle as `queries[i]` in one call; any invalid query fails the whole call.
...
- Evidence tools: `QueryAzure` for every HTTP evidence source (ARM, Microsoft Graph, Log Analytics/Application Insights, Blob Storage exports, Azure Retail Prices, official documentation/specs and public pages), plus `QueryUploadedFile` and `GetOperationStatus`. Host code owns only exact-host credential routing, security, scope, throttling, pagination, retention and redaction rules.
- Descriptions tell the model to look up rather than guess: `GET /subscriptions/{id}/providers/{namespace}?api-version=2021-04-01` for live apiVersions, the official OpenAPI specs in [Azure/azure-rest-api-specs](https://github.com/Azure/azure-rest-api-specs) (browse through the api.github.com contents API, read through raw.githubusercontent.com with `grepFor` or `resultQuery` keys/schema mode), [microsoftgraph/msgraph-metadata](https://github.com/microsoftgraph/msgraph-metadata) and learn.microsoft.com through `QueryAzure`. A failed call fails the evaluation, so verification comes before the call.
- The system prompt keeps a short list of API facts that prevent common failed calls (Cost Management grouping limits, Resource Graph scope/KQL rules, tenant-scope reservation listing, which needs tenant-level benefit reader roles and is called only for commitment questions). Keep it short and factual; do not grow it into per-question recipes.
...
- Tools return raw API JSON. `QueryAzure` converts successful CSV/XML to queryable JSON and accepts `resultQuery` so the complete retained result can be filtered, projected, grouped or counted in the same call. Large read-only results are retained for `QueryToolResult`; totals, counts, shares, rankings and comparisons over three or more values come from `QueryToolResult`, `CalculateCost`, `CompareAmounts` or `EstimateTokenCost`, never mental arithmetic.
...
- Public web reads go through `QueryAzure` public GET routing. The SSRF guard blocks literal private, loopback, link-local and metadata hosts before DNS, connects only to public IPs after DNS and redirects, disables proxy/cookies/auth, honors the host cancellation token and one deadline through body reading, and reports transport failure explicitly.
...
- Single and batched Resource Graph POST bodies require one explicit nonempty `subscriptions` or `managementGroups` array. Cost Management, Consumption and PolicyInsights paths require a scope prefix; tenant-root `reservationSummaries` is rejected in favour of discovered reservation-order or billing scopes.
- ARM transport repairs stay narrow and visible: a read whose api-version ARM rejects while listing supported versions is retried once with the newest listed stable version; an unlisted `UnsupportedApiVersion` tries at most two older stable versions from that provider's manifest; both annotate root `_apiVersion`. Model-authored batch JSON, read-only POST bodies and `resultQuery` go through `ModelJson`'s structural repair (unbalanced containers, stray closers, misplaced sibling objects, trailing bracket noise) but unterminated strings and non-container roots stay rejected. A subscription-prefixed Resource Graph path is routed to the provider path only when the body already lists that subscription. `HttpHelper` retries dropped connections for GET only, never for POST/PUT/PATCH. A Cost Management query's `Dimension` grouping named `Currency` (which the service rejects and which every row already carries as a column) is removed and reported in root `_request`; a `TagKey` named Currency is kept. Never widen these into guessing paths, scopes, bodies or values.
- Parallelize independent calls except Cost Management `/query` and `/forecast`, which are tenant-throttled. Use one `QueryAzure` `requests` batch for similar calls; the host forces any cost query/forecast batch sequential, retries once after a service delay of at most five minutes, keeps `retryAtUtc`/`willRetry` in cooldown SSE events, and stops after a final 429 for the rest of the turn. Cost Management permits at most two grouping dimensions; the host rejects more before dispatch.
...
- Retail prices are `QueryAzure` GETs to `https://prices.azure.com/api/retail/prices?currencyCode=USD&$filter=<OData>`. The host pins the origin, requires `$filter`, drops `$top`/`$skip`, defaults api-version and USD, follows only same-origin `NextPageLink` up to `maxPages` (1-10), retries 429/5xx and returns raw `Items` with `retrievedAtUtc`, `pages` and `complete`.
- Compare several regions or SKUs with `or` in one filter and use `resultQuery`/`QueryToolResult` to select compatible rows; a multi-service estimate fetches every component in one first-round batch. Some services, such as Load Balancer, publish only an `armRegionName 'Global'` rate. Meter, product and SKU names are not derivable from ARM SKUs; the model starts from structural fields and reads returned values instead of guessing.
- The prompt keeps pricing quality rules: clarify material configuration (OS/license, tier, fixed-region region) unless the user gave enough context or authorized assumptions, default to on-demand list prices when region and size are given, compare only within one product/meter and volume band, carry qualifiers into labels, and quote the retrieval time, which is not a price-effective date.
...
- Network Watcher `connectivityCheck` is an allowlisted `QueryAzure` ARM POST only when the body validates a probe from an existing VM resource ID to one destination address or resourceId and a TCP port from 1-65535. Never substitute connectivity from the app host or broaden it into a scan.
- `OperationStore` records intent before dispatch and preserves asynchronous state, polling URLs, retry deadlines and prerequisites. Use `GetOperationStatus`; an empty `operationId` lists up to 100 recent owner-bound operations in the current conversation. HTTP 202 is not success and unknown writes must not be retried blindly.
...
- Both deployment workflows also require `live-evaluations.yml`. Local and CI runs execute the same explicit set of 20 representative template/incident questions against real inference and Azure tools; all 20 must pass. Keep the complete template catalog available for coverage, but never accept a subset, arbitrary replacement set or changed contracts. This is curated coverage, not a verified popularity ranking. Any failed, missing, timed-out, wrong-revision or invalid verdict blocks deployment. Use the dedicated evaluation identity/environment for CI and only explicitly authorized maintainer-lab identity/scopes locally, never production credentials or customer data. `EVAL_DATA_CLASSIFICATION=synthetic` publishes answers; `internal-test` (maintainer-owned test tenants) publishes only questions, tool outcomes, timings and pass/fail, withholding answers, error text and judge rationale from summaries and artifacts. The judge sees tool arguments, visible structured outputs, the host's connection context and the full session timeline (agent model/effort, total and first-token seconds, per-call start/end/duration/round and success, concurrency, tool versus model time, throttle notices, and budgets) alongside the answer, and grades end-to-end efficiency with its Azure API knowledge. See `tests/LiveEvaluations/README.md` for configuration and test-host versus delegated-browser coverage. Do not skip unavailable prerequisites or relax rubrics to make a run pass. The suite waits out reported service cooldowns between cases and re-runs a case once, unchanged, only when it failed after a final service throttle; every other failure is final. Feature-branch runs pass `fail-fast: true`, stopping at the first final case failure to save time; production runs every case.
...
- Distinguish a valid negative judge verdict from malformed/missing judge output in gate diagnostics. Both fail the gate; acceptance requires the four boolean judge flags (`accepted`, `grounded`, `complete`, `efficient`) to be true, an integer `efficiencyScore` of at least 3 on the 1-5 scale, and a nonempty reason, without publishing private rationale. Efficiency failures are clear waste (failed, repeated, needlessly sequential, unfiltered-paging, unnecessary-lookup or unrelated calls); host-serialized Cost Management calls, throttle waits, needed discovery, exact result queries and configured reasoning time are not waste. Fix a flagged inefficiency in the agent prompt or tool description, never by loosening the judge.
