...
4. **Tool surface** — `AzureQueryTools` blocks DELETE + mutating POST allowlist enforced, `GraphQueryTools` blocks DELETE while standard consent tiers remain read-only, `LogAnalyticsQueryTools` query-only, `UploadedFileTools` path-sandboxed (no `..` / symlink escape, size + type checked), no SSRF (block the cloud metadata endpoint, loopback, and RFC1918 ranges), `HttpHelper` never logs Authorization or token bodies.
