...
4. **Tool surface** — `AzureQueryTools` blocks DELETE, routes credentials by exact host, enforces the ARM mutating POST allowlist, turns PUT/PATCH into approval proposals, keeps standard Graph consent tiers read-only, restricts Log Analytics/Application Insights to query endpoints, and uses `PublicWebReader` SSRF protections (cloud metadata, loopback, private/link-local ranges blocked before DNS and at connect time); `UploadedFileTools` is path-sandboxed (no `..` / symlink escape, size + type checked), and `HttpHelper` never logs Authorization or token bodies.
