...
- **Organizational Knowledge Base (#28)** — Users can save persistent context about their environment once (subscription→app mappings, cost-center owners, SLA/RTO/RPO targets, tagging conventions, fiscal calendar, custom analysis instructions) and the agent applies it automatically across every conversation. Stored per-user on the durable `/home` mount (`$COPILOT_HOME/knowledge/{userId}/knowledge.json`), so it survives container restarts and redeploys. Entra-only (anonymous sessions excluded). New `KnowledgeStore` service, `KnowledgeEndpoints` CRUD + file-import API (`/api/knowledge`), a sidebar Knowledge Base UI with editor modal and text-file import, and a read-only `QueryKnowledge` tool. Token cost is bounded by a two-tier injection strategy: inject-once-per-session (re-inject only on change or every 10 turns) and, above a 4K-char budget, switch to a compact index + lazy-pull via `QueryKnowledge`. See `docs/knowledge-base.md` and `docs/knowledge-authoring-guide.md`.
