...
- **`QueryGraph` is now GET-only at the code level.** Every consented Microsoft Graph scope is read-only and the consent UI promises "read-only by scope definition", but the tool previously allowed POST/PUT/PATCH (only DELETE was blocked). Non-GET requests are now rejected with HTTP 403 as defense in depth, and the tool description no longer advertises write methods.
