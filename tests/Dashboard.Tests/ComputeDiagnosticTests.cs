...
public sealed class ComputeDiagnosticTests
{
    ...
    {
        resourceType = "virtualMachines", name = "Standard_Test", family = "testFamily", locations = new[] { region },
        locationInfo = new[] { new { location = region, zones = new[] { "1", "2" } } },
        capabilities = new[] { new { name = "vCPUs", value = (string?)"8" }, new { name = "LowPriorityCapable", value = lowPriorityCapable } }, restrictions
    });
    ...
    [InlineData(false)]
    [InlineData(true)]
    public async Task RegisteredSpotToolRetainsLateCatalogueRowsAndQueriesEveryAdvertisedRegion(bool missingRegionalHistory)
    {
        var requests = new List<(string Url, HttpMethod Method)>();
        ...
        {
            ["subscriptionsJson"] = "[\"00000000-0000-0000-0000-000000000001\"]",
            ["skuNames"] = "Standard_Test", ["regions"] = "all", ["priority"] = "spot", ["count"] = "2"
        });
