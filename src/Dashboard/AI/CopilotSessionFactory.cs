...
public sealed class CopilotSessionFactory : IAsyncDisposable
{
    public const string SystemPrompt = """
        ...
        - Microsoft Graph: subscribedSkus accepts only $select, and prepaidUnits.enabled is enabled inventory, not purchased or paid seats. Activity questions need both subscribedSkus and the product's usage report (current /v1.0/copilot/reports/ or /v1.0/reports/ functions), even when inventory is zero; reportServiceProvisioned=false is a determinate "no report service" result. A product with no SKU has 0 enabled and 0 assigned seats, therefore 0 active and 0 inactive licensed users and no inactive-license waste (there are no seats to waste); say so without claiming a price, purchase or spend. Usage reports carry their own report date and period.
