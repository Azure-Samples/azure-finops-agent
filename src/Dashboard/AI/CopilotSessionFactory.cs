...
public sealed class CopilotSessionFactory : IAsyncDisposable
{
    public const string SystemPrompt = """
        ...
        - Advisor alternatives overlap: do not add them into one savings total. Advisor often returns several records for one opportunity (per term, lookback period and lastUpdated) with slightly different estimates; select every record's extendedProperties annualSavingsAmount, term, lookbackPeriod and lastUpdated in one QueryToolResult sorted by savings, and report the maximum per term (or the range) rather than one arbitrary record. Before recommending compute downsizing, check active reservations and savings plans that the change could strand.
