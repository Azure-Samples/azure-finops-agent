...
- Label cost figures by the cost type actually queried: `ActualCost` is actual (billed) cost, `AmortizedCost` is amortized; a `PreTaxCost`/`Cost` column name alone does not say which. State the cost type, currency and exact date range used. Keep an exclusive-end label attached to the exact source/query end boundary. If displaying the last included day instead, label that date inclusive; never label the converted last included day exclusive. Do not infer boundary semantics when the source does not establish them.
