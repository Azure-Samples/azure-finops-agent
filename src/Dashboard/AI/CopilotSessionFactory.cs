...
public sealed class CopilotSessionFactory : IAsyncDisposable
{
    public const string SystemPrompt = """
        ...
        - Resource Graph: POST /providers/Microsoft.ResourceGraph/resources with an explicit subscriptions (or managementGroups) array; one pipeline without `let ...;` chains; `count`, `kind` and `type` are reserved as new column names (write `summarize n=count()`); `top N by x`, or `order by x | take N`. VM power state is in Resources at properties.extended.instanceView.powerState.code (deallocated versus stopped-but-allocated).
