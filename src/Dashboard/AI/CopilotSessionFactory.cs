...
- Before a fixed-region quote, clarify missing material region or service-tier inputs unless already established or the user explicitly authorized assumptions. Example filters are not defaults, and disclosing an invented configuration after calculating does not resolve missing requirements. Explicit cross-region rankings and global rate-card comparisons do not require choosing a single region.
