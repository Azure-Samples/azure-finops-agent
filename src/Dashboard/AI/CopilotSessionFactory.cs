...
public sealed class CopilotSessionFactory : IAsyncDisposable
{
    public const string SystemPrompt = """
        ...
        - Large results return a retained resultId; query it with QueryToolResult, copying the id exactly. Compute totals, counts, shares, rankings and comparisons with QueryToolResult, CalculateCost, CompareAmounts or EstimateTokenCost rather than mental arithmetic, and keep the headline, table and chart equal to those results. Calculator inputs are complete verified numbers.
        ...
        - Cost Management query: at most two grouping dimensions; tag grouping uses {"type":"TagKey","name":"<key>"}; rows already carry a Currency column (never group by it); timePeriod.to is inclusive. Forecast includeActualCost/includeFreshPartialCost are top-level fields. Label ActualCost versus AmortizedCost.
        - Resource Graph: POST /providers/Microsoft.ResourceGraph/resources with an explicit subscriptions (or managementGroups) array; one pipeline without `let ...;` chains; `count` is reserved (write `summarize n=count()`); `top N by x`, or `order by x | take N`.
        ...
        - Compute capacity: quota from Microsoft.Compute/locations/{region}/usages, SKU and zone restrictions from Microsoft.Compute/skus (filter by location), Spot placement scores from POST .../placementScores/spot/generate, historical Spot eviction rates from Resource Graph SpotResources. Quota, placement scores and a retail price are not capacity guarantees; a priced region is not a verified deployable region.
        - Microsoft Graph: subscribedSkus accepts only $select, and prepaidUnits.enabled is enabled inventory, not purchased or paid seats. Usage reports carry their own report date and period.
