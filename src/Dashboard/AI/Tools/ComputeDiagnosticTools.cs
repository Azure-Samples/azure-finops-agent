...
private async Task<string> CheckComputeFeasibility(
    [Description("JSON-encoded array of 1-10 subscriptions from connection context. Accepts GUID strings or objects with a string id and optional name, for example [{\"id\":\"subscription-guid\",\"name\":\"display-name\"}]. Preserve every requested subscription; pass the encoded JSON as a string.")] string subscriptionsJson,
    ...
    CancellationToken cancellationToken = default)
{
    ...
    try
    {
        using var document = JsonDocument.Parse(subscriptionsJson);
        if (document.RootElement.ValueKind != JsonValueKind.Array || document.RootElement.GetArrayLength() is < 1 or > 10)
            return "Error: subscriptionsJson must encode an array of 1-10 subscription GUID strings or objects with a string id.";
        subscriptions = document.RootElement.EnumerateArray().Select(item => item.ValueKind == JsonValueKind.String
            ? item.GetString() ?? ""
            : item.ValueKind == JsonValueKind.Object && item.TryGetProperty("id", out var id) && id.ValueKind == JsonValueKind.String
                ? id.GetString() ?? "" : "").ToArray();
    }
    catch (JsonException) { return "Error: subscriptionsJson must encode a JSON array of subscription GUID strings or objects with a string id."; }
    ...
    foreach (var subscription in subscriptions.Distinct(StringComparer.OrdinalIgnoreCase))
    {
        ...
        using var quotaLimiter = new SemaphoreSlim(6, 6);
        var regionalUsages = await Task.WhenAll(locations.Select(async location =>
        {
            await quotaLimiter.WaitAsync(cancellationToken);
            try
            {
                ...
                return (Location: location, Usage: usage);
            }
            finally { quotaLimiter.Release(); }
        }));
        foreach (var (location, usage) in regionalUsages)
        {
