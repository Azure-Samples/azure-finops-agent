...
public sealed class OperationTools(UserTokens tokens)
{
    ...
    public IEnumerable<AIFunction> Create()
    {
        yield return AIFunctionFactory.Create(GetOperationStatus, "GetOperationStatus",
            "Checks host-registered Azure operations (approval proposals, 202 accepted diagnostics and report downloads such as pricesheets). With an exact opaque operationId it polls only the stored ARM URL under the caller's delegated token; it never accepts a URL or repeats a mutation. Respect nextPollUtc, reuse terminal results, and do not poll unrelated operations. Accepted, unknown and in-progress operations are not completed. Without operationId it lists up to 100 recent owner-bound operations in this conversation, including prerequisite resources from partial deployments (not an all-Azure inventory); use that after a failed multi-resource change to generate a reviewed cleanup script. Never delete automatically.");
    }

    private async Task<string> GetOperationStatus([Description("Exact opaque operationId returned by the earlier mutation or diagnostic being checked. Omit to list this conversation's operations.")] string operationId = "",
        CancellationToken cancellationToken = default)
    {
        if (string.IsNullOrWhiteSpace(operationId)) return ListOperationResults();
